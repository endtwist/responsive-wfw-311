...
int PASCAL WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmd, int nShow)
{
    ...
    hText = CreateWindow("edit", pText,
                         ...
                         /* NOT disabled: a disabled control ignores the mouse, scroll bar and all,
                            which made the note unscrollable with a real pointer. Read-only keeps
                            it uneditable; on the phone the host's scroll policy means a drag
