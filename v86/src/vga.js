...
export function VGAScreen(cpu, bus, screen, vga_memory_size)
{
    ...
    this.pv_sel = 0;
    this.pv_socks = [];
    for(let i = 0; i < 8; i++) this.pv_socks.push(pv_sock_blank());
    ...
    bus.register("pv-sock-data", function(d) {
        const k = this.pv_socks[(d.handle | 0) & 7];
        if(!k || k.state === 0) return;                         // closed while the fetch was running
...
VGAScreen.prototype.port1CF_write = function(value)
{
    ...
    switch(this.dispi_index)
    {
        ...
        case 0x30:
            ...
            else if(value === 2) this.pv_sock_read(this.pv_sock_cur().arg);
            else if(value === 3) this.pv_sock_close();
            else if(value === 4) this.pv_sock_send(this.pv_sock_cur().arg);
            break;
        case 0x31:
            this.pv_sock_cur().arg = value & 0xFFFF;
            break;
        case 0x35:
            // PV SOCKET CHAR: one byte of the target, appended; CMD 1 opens what has accumulated
            if(this.pv_sock_cur().str.length < 2048) this.pv_sock_cur().str += String.fromCharCode(value & 0xFF);
            break;
        case 0x36:
            // PV SOCKET SEL: which of the eight handles every other register acts on
            this.pv_sel = value & 7;
            break;
        case 0x37:
            // PV SOCKET SEND: two more bytes of the outbound stream, little endian
            {
                const k = this.pv_sock_cur();
                if(k.out.length < 0x20000) { k.out.push(value & 0xFF); k.out.push((value >> 8) & 0xFF); }
            }
            break;
        case 0x1B:
...
{
    return this.pv_socks[this.pv_sel & 7];
};
VGAScreen.prototype.pv_sock_open = function()
{
    /* The target accumulated through the CHAR register: either a URL, which the host fetches at
       once, or a "host:port", which opens a stream and waits for the request the guest will send.
       Both end up in the same place -- the host performs the exchange and the reply becomes this
       handle's read stream -- which is what lets one device serve both a program that knows about
       it and a Winsock program that does not. */
    const k = this.pv_sock_cur();
    const target = k.str;
    k.str = "";
    if(!target) { k.result = 1; return; }
    k.state = 1; k.result = 0; k.buf = null; k.off = 0; k.block = 0; k.done = false; k.out = [];
    this.bus.send("pv-sock-open", { handle: this.pv_sel & 7, target });
};
VGAScreen.prototype.pv_sock_send = function(len)
{
    /* The bytes the guest wrote through the SEND register, handed up as one write. The host decides
       when it has a complete request -- the same rule web/net.js already uses on the SLIP line. */
    const k = this.pv_sock_cur();
...
{
    const h = this.pv_sel & 7;
    this.pv_socks[h] = pv_sock_blank();
    this.bus.send("pv-sock-close", { handle: h });
};
...
VGAScreen.prototype.svga_register_read = function(n)
{
    switch(n)
    {
        ...
        case 0x32:
            return this.pv_sock_cur().result & 0xFFFF;   // bytes staged by the last read
        case 0x33:
            return this.pv_sock_cur().state & 0xFFFF;     // 0 idle 1 fetching 2 ready 3 complete 0xFF error
        case 5:
